...
const Navbar = ({ onLogout }) => {
  ...
  const handleLogout = () => {
    ...
    onLogout(); // Optional callback for additional logout handling (e.g., updating global state)
  ...
  return (
    <nav className={styles.navbar}> {/* Navbar container */}
      ...
      <div className={styles['employee-name']}> {/* Display employee name */}
        <strong>{employeeName}</strong> {/* Show the employee's name or 'User' */}
      </div>
      {/* Logout button */}
